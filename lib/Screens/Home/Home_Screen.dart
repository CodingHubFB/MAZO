...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future getCurrentMerchant(userId) async {
    var currentUser = await AppUtils.makeRequests(
      "fetch",
      "SELECT id, Fullname, urlAvatar FROM Users WHERE uid = '$userId'",
    );
    print("UUUUUUU ${currentUser}");
  ...
  Future getItems() async {
    var itemsx = await AppUtils.makeRequests(
      "fetch",
      "SELECT id, name, price, description, media,comments,uid, created_at FROM Items WHERE visibility = 'Public'",
    );
    ...
    return Scaffold(
      ...
      body: PageView.builder(
        ...
        onPageChanged: (index) async {
          ...
          // أوقف الفيديو اللي كان شغال قبل كدا
          ...
          await AppUtils.makeRequestsViews(
            "query",
            "UPDATE Items SET Views = Views + 1 WHERE id = '${Provider.of<AppProvider>(context, listen: false).itemId}'",
          );
          getCountLikes(
            Provider.of<AppProvider>(context, listen: false).itemId,
          );
          getUserLike(Provider.of<AppProvider>(context, listen: false).itemId);
